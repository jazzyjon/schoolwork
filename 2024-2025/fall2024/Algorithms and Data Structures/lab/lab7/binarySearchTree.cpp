...
Node *select(Node *T, int r)
{
	...
	int sizeL = T->left ? T->left->size : 0;
	...
	return select(T->right, r - sizeL - 1);
}
...
Node *join(Node *L, Node *R)
{
	...
	int sizeT = L->size + R->size;

	if (rand() % sizeT < L->size)
	{
...
Node *remove(Node *T, int k)
{
	...
	{
		Node *combi = join(T->left, T->right);
